...
AS $$
DECLARE
  v_coupon marketing_coupons;
  v_campaign marketing_campaigns;
  v_discount_value numeric;
BEGIN
  -- Get coupon and campaign details
  SELECT c.*, mc.* 
  INTO v_coupon, v_campaign
  ...
  -- Calculate discount
  CASE v_campaign.type
    WHEN 'amount_off' THEN
      v_discount_value := v_campaign.discount_value;
    WHEN 'percentage_off' THEN
      v_discount_value := (p_total_bill * v_campaign.discount_value / 100);
    WHEN 'free_item' THEN
