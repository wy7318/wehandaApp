...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const signIn = async (email: string, password: string) => {
    try {
      ...
      // Store credentials for potential biometric use
      if (!error) {
  ...
  const enableBiometrics = async () => {
    ...
    if (!lastCredentials) {
      throw new Error('Please sign in first before enabling biometrics');
    }
    ...
    if (result.success && isMounted.current) {
      ...
      const credentials = JSON.stringify(lastCredentials);
  ...
  const signInWithBiometrics = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        return { error: new Error('No stored credentials found. Please sign in normally and re-enable biometrics.') };
      } else {
        return { error: new Error('Biometric authentication failed') };
      }
    } catch (error) {
